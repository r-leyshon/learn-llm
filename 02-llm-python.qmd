...
Positional encoding gets added to embeddings. 
...
class PositionalEncoder(torch.nn.Module):
    def __init__(self, dim_mod: int, max_len: int) -> None:
        ...
        div_term = torch.exp(
            torch.arange(0, dim_mod, 2, dtype=torch.float) * -(math.log(10000.0) / dim_mod)
            )
...
# v.brittle, if dim_mod does not match length of tensor, RuntimeError. Refactor to extract len from tensor...
